...
const handleY: TileColliderHandler = ({ entity, match, resolver, gameContext, level }) => {
  if (entity.vel.y > 0) {
    ...
    if (entity.getTrait(Player)) {
      ...
      const goombaBrown = gameContext.entityFactory['goomba-brown']!();
      goombaBrown.vel.set(50, -400);
      goombaBrown.pos.set(entity.pos.x, match.y1);
      level.entities.add(goombaBrown);
    }
